...
import { Screen, Card, ActivityIndicator } from '../components'
import colors from '../config/colors'
import postApi from '../api/post'
import useAuth from '../auth/useAuth'
import moment from 'moment'
import SearchBar from '../components/SearchBar'
import { ListItem, DeleteAction } from '../components/Lists'
...
function PostListScreen ({ navigation }) {
  ...
  const handleDelete = post => {
    const newPost = Object.values(post).filter((p) => p.id !== post.post_id).map(({ post_id, user_id }) => ({ post_id, user_id }))
